...
import Biblioteca.MapaAbstrata;
 ...
 * @author basmoura
 */
public class ListaMapa<K extends Comparable<K>, V>
        extends MapaAbstrata<K, V> {

    private ListaEncOrd<ChaveValor<K, V>> mapa = new ListaEncOrd<>();
    ...
    public V put(K chave, V valor) {
        ...
        ChaveValor<K, V> obj = new ChaveValor<>(chave, valor);
    ...
    public V remove(K chave) {
        ChaveValor<K, V> chaveValor = new ChaveValor<>(chave, null);
    ...
    public boolean contains(K chave) {
        ChaveValor<K, V> entrada = new ChaveValor<>(chave, null);
