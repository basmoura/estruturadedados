package lista04_ArvoreBusca;
import java.util.Comparator;
import utilitarios.*;

public class Demo {
	
	static ArvoreBusca<Aluno> alunos;
	static ArvoreBusca<Disciplina> disciplinas;
	static ArvoreBusca<Turma> turmas;
	...
	private static void matricularAluno() {
		...
		Keyboard.clrscr();
		do {
			int matricula = Keyboard.readInt("Entrar com o numero de matrícula: ");
	...
	private static void incluirAluno() {
		...
		do {
			...
			System.out.println();
			resp = Keyboard.readChar("Outra inclusão (s/n)? ");
		} while (resp == 's');
	}
	...
	private static void excluirAluno() {
		...
		Keyboard.clrscr();
		do {
			int matricula = Keyboard.readInt("Entrar com a matrícula: ");
	...
	private static void lerArquivos() {
		if (arqUniversidade.reset()) {
			alunos = (ArvoreBusca<Aluno>) arqUniversidade.read();
			disciplinas = (ArvoreBusca<Disciplina>) arqUniversidade.read();
			turmas = (ArvoreBusca<Turma>) arqUniversidade.read();
			arqUniversidade.closeFile();
		} else {
			alunos = new ArvoreBusca<>();
			disciplinas = new ArvoreBusca<>();
			turmas = new ArvoreBusca<>();
		}
	}
}
