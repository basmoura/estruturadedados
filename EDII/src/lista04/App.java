...
public class App {
	...
	private static ArvoreBusca<Aluno> alunos = new ArvoreBusca<Aluno>();
	private static ArvoreBusca<Disciplina> disciplinas = new ArvoreBusca<Disciplina>();
	private static ArvoreBusca<Turma> turmas = new ArvoreBusca<Turma>();
	...
	static void listarTurmas() {
		...
		Turma p = it.getFirst();
		Turma[] arrayTurmas = new Turma[turmas.size()];
		int cont = 0;
		while (p != null) {
			arrayTurmas[cont] = p;
			cont++;
	...
	static void listarAlunosMatriculados() {
		...
		if (disciplina == null) {
			...
			if (turma == null) {
				...
				System.out.println("------  --------------------------");
				Aluno[] arrayAlunos = new Aluno[alunos.size()];
				int cont = 0;
				MyIterator<Aluno> it = alunos.iterator();
				Aluno p = it.getFirst();
				while (p != null) {
