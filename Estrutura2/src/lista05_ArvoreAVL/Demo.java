package lista05_ArvoreAVL;
import java.util.Comparator;  
import utilitarios.*;

public class Demo {
	
	static ArvoreAVL<Aluno> alunos;
	static ArvoreAVL<Disciplina> disciplinas;
	static ArvoreAVL<Turma> turmas;
	...
	private static void lerArquivos() {
		if (arqUniversidade.reset()) {
			alunos = (ArvoreAVL<Aluno>) arqUniversidade.read();
			disciplinas = (ArvoreAVL<Disciplina>) arqUniversidade.read();
			turmas = (ArvoreAVL<Turma>) arqUniversidade.read();
			arqUniversidade.closeFile();
		} else {
			alunos = new ArvoreAVL<>();
			disciplinas = new ArvoreAVL<>();
			turmas = new ArvoreAVL<>();
		}
	}
}
